...
static const char *truecolor_terminals[] = {
    "iTerm.app",
    "Apple_Terminal",  /* macOS Terminal.app (10.14+) */
    "Hyper",
    "vscode",
    "Alacritty",
    "kitty",
...
static const char *term_256color_patterns[] = {
    "-256color",
    "-256colour",
    NULL
};
...
void termcaps_set_query_enabled(int enabled) {
    g_query_enabled = enabled;
}

int termcaps_get_query_enabled(void) {
    return g_query_enabled;
}
...
const char *termcaps_format_fg_color(int r, int g, int b) {
    ...
    switch (g_caps.color_level) {
        case TERM_COLOR_TRUECOLOR:
            snprintf(g_fg_color_buf, sizeof(g_fg_color_buf),
...
const char *termcaps_format_bg_color(int r, int g, int b) {
    ...
    switch (g_caps.color_level) {
        case TERM_COLOR_TRUECOLOR:
            snprintf(g_bg_color_buf, sizeof(g_bg_color_buf),
...
const char *termcaps_describe(void) {
    ...
    const char *color_names[] = {
        "none", "8-color", "16-color", "256-color", "truecolor"
    };
    const char *unicode_names[] = {
        "ASCII", "basic (Latin-1)", "full UTF-8"
    };

    int color_idx = (int)g_caps.color_level;
    int unicode_idx = (int)g_caps.unicode_level;

    if (color_idx < 0 || color_idx > 4) color_idx = 0;
    if (unicode_idx < 0 || unicode_idx > 2) unicode_idx = 0;
