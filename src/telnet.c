...
int telnet_connect(Telnet *t, const char *hostname, int port) {
  ...
  if (keepalive_env) {
    LispObject *enable_keepalive =
        env_lookup(keepalive_env, "*tcp-keepalive-enabled*");
    if (enable_keepalive && enable_keepalive != NIL &&
        lisp_is_truthy(enable_keepalive)) {
      /* Get keepalive timing from Lisp variables (defaults: 60s idle, 10s
       * interval) */
      ...
      LispObject *interval_obj =
          env_lookup(keepalive_env, "*tcp-keepalive-interval*");
      ...
      if (setsockopt(t->socket, SOL_SOCKET, SO_KEEPALIVE, (char *)&optval,
        ...
        ka.keepaliveinterval =
            keepalive_interval * 1000; /* Convert to milliseconds */
