...
(defun tintin-save-state (filename)
  (let ((file (open filename "w")))
    ...
    (write-line file "")
    ;; Write custom colors (before aliases/highlights that may reference them)
    (write-line file ";; Colors")
    (let ((color-entries (hash-entries *tintin-custom-colors*)))
      (do ((i 0 (+ i 1))) ((>= i (length color-entries)))
        (let* ((entry (list-ref color-entries i))
               (name (car entry))
               (spec (cdr entry)))
          (write-line file
           (concat "(hash-set! *tintin-custom-colors* " "\""
            (tintin-escape-string name) "\" " "\"" (tintin-escape-string spec)
            "\")")))))
    (write-line file "")
    ;; Write aliases
